...
def connection1():
    conn = pymysql.connect(
        host='localhost', user='root', password='', db='users'
    )
    return conn
...
class App(customtkinter.CTk):
    def __init__(root, *args, **kwargs):
        ...
        def search_event():
            ...
            WHERE MEMID=%s  OR FNAME=%s OR LNAME=%s  OR ADDRESS=%s OR PHONE=%s OR PHONE1=%s"""
            try:
                cursor.execute(query, (memid, fname, lname, address, phone, phone1))
