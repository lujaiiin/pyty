...
class App(customtkinter.CTk):
    def __init__(root, *args, **kwargs):
        ...
        def search_event():
            ...
            conn= connection()
            cursor=conn.cursor()
            cursor.execute("SELECT * FROM member_doctor WHERE MEMID='"+
                        memid+"' or FNAME='"+
                        fname+"' or LNAME='"+
                        lname+"' or ADDRESS='"+
                        address+"' or PHONE='"+
        ...
        root.sidebar_button_2 = customtkinter.CTkButton(root.sidebar_frame, text="دكتوراه", command=root.sidebar_button_event)
