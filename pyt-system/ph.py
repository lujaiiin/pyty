...
class App(customtkinter.CTk):
    def __init__(root, *args, **kwargs):
        ...
        def read():
            conn = connection()
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM member_doctor ")
        ...
        def add_event():
            ...
            if (memid=="" or memid==" ") or (fname=="" or fname==" ") or (lname=="" or lname==" ") or (address=="" or address==" ") or (phone=="" or phone==" ") or (phone1=="" or phone1==" ") or (phone2=="" or phone2==" ")or (degry=="" or degry==" ")or (salary=="" or salary==" "):
                ...
                try:
                    ...
                    cursor.execute("INSERT INTO member_doctor VALUE ('"+memid+"', '"+fname+"', '"+lname+"', '"+address+"', '"+phone+"', '"+phone1+"', '"+phone2+"', '"+degry+"', '"+salary+"') ")
                    conn.commit()
                    conn.close()
                except: 
        ...
        def delete_selected_item():
            ...
            try:
                ...
                cursor.execute("DELETE FROM member_doctor WHERE MEMID='" + str(deleteData) + "'")
                conn.commit()
                conn.close()
                refreshTable()
        ...
        def update_event():
            ...
            if (memid=="" or memid==" ") or (fname=="" or fname==" ") or (lname=="" or lname==" ") or (address=="" or address==" ") or (phone=="" or phone==" ") or (phone1=="" or phone1==" ") or (phone2=="" or phone2==" ") or (salary=="" or salary==" ") or (degry=="" or degry==" "):
                ...
                try:
                    conn= connection()
                    cursor=conn.cursor()
                    cursor.execute("UPDATE member_doctor SET MEMID='"+
                    memid+"', FNAME='"+
                    fname+"', LNAME='"+
                    lname+"', ADDRESS='"+
                    address+"', PHONE='"+
        ...
        """def reset_event():
            ...
            if desicion != "yes":
                ...
                try:
                    conn= connection()
                    cursor=conn.cursor()
                    cursor.execute("DELETE FROM member_doctor")
                    conn.commit()
                    conn.close()
                except:
        ...
        # configure window
        root.title("اعضاء هيئة التدريس(دكتوراه)")
        ...
        root.sidebar_button_4 = customtkinter.CTkButton(root.sidebar_frame, text="ماجستير", command=button4_command)
        root.sidebar_button_4.grid(row=4, column=0, padx=20, pady=10)

        root.sidebar_button_5 = customtkinter.CTkButton(root.sidebar_frame, text="المتعاونين", command=button5_command)
