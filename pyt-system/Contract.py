...
class App(customtkinter.CTk):
    def __init__(root, *args, **kwargs):
        ...
        def search_event():
            ...
            query = """
            SELECT * FROM member_mo WHERE MEMID=%s OR FNAME=%s OR LNAME=%s OR ADDRESS=%s OR PHONE=%s OR PHONE1=%s OR PHONE2=%s OR DEGRY=%s OR SALARY=%s
            """
            params = (memid, fname, lname, address, phone, phone1, phone2, degry, salary)
            ...
            try:
                ...
                tkinter.messagebox.showinfo("Error", f"Database error: {e}")
            finally:
                conn.close()
        ...
        root.sidebar_button_2 = customtkinter.CTkButton(root.sidebar_frame, text=" متعاونين", command=root.sidebar_button_event)
