...
class App(customtkinter.CTk):
    def __init__(root, *args, **kwargs):
        ...
        def read():
            conn = connection()
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM member_mo")
        ...
        def add_event():
            ...
            if (memid=="" or memid==" ") or (fname=="" or fname==" ") or (lname=="" or lname==" ") or (address=="" or address==" ") or (phone=="" or phone==" ") or (phone1=="" or phone1==" ") or (phone2=="" or phone2==" ")or (degry=="" or degry==" ")or (salary=="" or salary==" "):
                ...
                try:
                    ...
                    cursor.execute("INSERT INTO member_mo VALUE ('"+memid+"', '"+fname+"', '"+lname+"', '"+address+"', '"+phone+"', '"+phone1+"', '"+phone2+"', '"+degry+"', '"+salary+"') ")
                    conn.commit()
                    conn.close()
                except: 
        ...
        def delete_event():
            ...
            if desicion != "yes":
                ...
                db_password = get_password_from_db()
        ...
        def delete_selected_item():
            ...
            try:
                ...
                cursor.execute("DELETE FROM member_mo WHERE MEMID='" + str(deleteData) + "'")
                conn.commit()
                conn.close()
                refreshTable()
            except Exception as e:
                messagebox.showinfo("Error", "SORRY an error occurred: " + str(e))
                return
        ...
        def select_event():
            try:
                ...
                root.input_box_1.delete(0, tk.END)  
                root.input_box_1.insert(0, values[0])  

                root.input_box_2.delete(0, tk.END)  
                root.input_box_2.insert(0, values[1])  

                root.input_box_3.delete(0, tk.END)  
                root.input_box_3.insert(0, values[2])  

                root.input_box_4.delete(0, tk.END) 
                root.input_box_4.insert(0, values[3])  

                root.input_box_5.delete(0, tk.END)  
                root.input_box_5.insert(0, values[4])  

                root.input_box_6.delete(0, tk.END)  # Clear the current text
                root.input_box_6.insert(0, values[5])  # 

                root.input_box_7.delete(0, tk.END)  # Clear the current text
                root.input_box_7.insert(0, values[6])  # 
                
                root.input_box_8.delete(0, tk.END)  # Clear the current text
                root.input_box_8.insert(0, values[7])  # 
                
                root.input_box_9.delete(0, tk.END)  # Clear the current text
                root.input_box_9.insert(0, values[8])  # 
        ...
        def search_event():
            ...
            cursor = conn.cursor()
            query = """
            SELECT * FROM member_mo  
            WHERE MEMID=%s  OR FNAME=%s OR LNAME=%s  OR ADDRESS=%s OR PHONE=%s OR PHONE1=%s OR phone2=%s OR DEGRY =%s OR SALARY=%s"""
            try:
                cursor.execute(query, (memid, fname, lname, address, phone, phone1, phone2, degry, salary))
                result = cursor.fetchall()
                if result:
                    for item in root.mt_trea.get_children():
                        root.mt_trea.delete(item)
                    for row in result:
                        root.mt_trea.insert('', 'end', values=row)
                else:
        ...
        def update_event():
            ...
            if (memid=="" or memid==" ") or (fname=="" or fname==" ") or (lname=="" or lname==" ") or (address=="" or address==" ") or (phone=="" or phone==" ") or (phone1=="" or phone1==" ") or (phone2=="" or phone2==" ") or (salary=="" or salary==" ") or (degry=="" or degry==" "):
                ...
                try:
                    conn= connection()
                    cursor=conn.cursor()
                    cursor.execute("UPDATE member_mo SET MEMID='"+
                    memid+"', FNAME='"+
                    fname+"', LNAME='"+
                    lname+"', ADDRESS='"+
                    address+"', PHONE='"+
        ...
        def clear_event():
            try:
                ...
                root.input_box_4.delete(0, tk.END)  
                root.input_box_5.delete(0, tk.END) 
                root.input_box_6.delete(0, tk.END)
                root.input_box_7.delete(0, tk.END) 
                root.input_box_8.delete(0, tk.END)
                root.input_box_9.delete(0, tk.END)
                refreshTable()  
            except IndexError:
                messagebox.showinfo("Error", "retry")
        ...
        """def reset_event():
            ...
            if desicion != "yes":
                ...
                try:
                    conn= connection()
                    cursor=conn.cursor()
                    cursor.execute("DELETE FROM member_mo")
                    conn.commit()
                    conn.close()
                except:
        ...
        # configure window
        root.title("اعضاء هيئة التدريس (متعاونين)")
        ...
        root.sidebar_button_5 = customtkinter.CTkButton(root.sidebar_frame, text="ماجستير", command=button5_command)
        ...
        root.mt_trea.heading("Address", text="الدرجة العلمية ", anchor="w")
        ...
        root.mt_trea.heading("Phone3", text=" الدرجة الاكاديمية", anchor="w")
        ...
        root.input_box_4 = ctk.CTkEntry(root.main_content_frame, placeholder_text=" الدرجة العلمية ")
        ...
        root.input_box_8 = ctk.CTkEntry(root.main_content_frame, placeholder_text=" الاكاديمية الدرجة ")
        ...
        root.reset_button = ctk.CTkButton(root.main_content_frame, text=" refresh", command=clear_event)
